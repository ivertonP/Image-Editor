...
public class FramePrincipal extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jMenu2.setActionCommand("Transformadas");
        ...
        jMenu2.getAccessibleContext().setAccessibleName("Transformadas");
        ...
        jMenu4.getAccessibleContext().setAccessibleName("Histogramas");
    ...
    private static BufferedImage RedimensionarImagem(){
        ...
        double alturaTela = screenSize.getHeight();
        double larguraTela = screenSize.getWidth();
        alturaTela = 0.80 * alturaTela;                                                             //a constante 0.80 representa a porcentagem de pixels necessários para preservar o layout da GUI
        larguraTela = 0.97 * larguraTela;                                                           //a constante 0.97 representa a porcentagem de pixels necessários para preservar o layout da GUI
        ...
        if((larguraImg <= larguraTela) && (alturaImg <= alturaTela)){                               //Imagem com altura e largura menores ou iguais às dimensões da tela
            image = imagemCarregadaDoDisco.getScaledInstance(larguraImg, alturaImg, 100);            
            ConstruirImagem();
        }
        else{
            if(alturaTela > larguraTela){                                                           //Telas verticalmente retangulares
                ...
                image = imagemCarregadaDoDisco.getScaledInstance((int)novaLarguraImg, (int)novaAlturaImg, 100);      //o método getScaledInstance() somente aceita parâmetros do tipo int           
                ConstruirImagem();
            }
            else{
                if(larguraTela > alturaTela){                                                       //Telas horizontalmente retangulares
                    ...
                    image = imagemCarregadaDoDisco.getScaledInstance((int)novaLarguraImg, (int)novaAlturaImg, 100);            
                    ConstruirImagem();
                }
    ...
    private static void ConstruirImagem(){   
        ...
        imagemOriginalRedimensionadaSemAlteracao = new BufferedImage(image.getWidth(null), image.getHeight(null), BufferedImage.TYPE_INT_RGB);
        ...
        Graphics2D bGr3 = imagemOriginalRedimensionadaSemAlteracao.createGraphics();
        Graphics2D bGr4 = imagemOriginalRedimensionadaParaReversao.createGraphics();
        bGr1.drawImage(image, 0, 0, null);
        bGr2.drawImage(image, 0, 0, null);
        bGr3.drawImage(image, 0, 0, null);
        bGr4.drawImage(image, 0, 0, null);
        bGr1.dispose();
        bGr2.dispose();
        bGr3.dispose();
        bGr4.dispose();        
        ...
        YIQ.estruturaTemporaraCopia = new YIQ.EstruturaYIQ[imagemASerExibida.getHeight()][imagemASerExibida.getWidth()];
        for (int j = 0; j < imagemASerExibida.getWidth(); j++) {
            for (int i = 0; i < imagemASerExibida.getHeight(); i++) {
                ...
                YIQ.estruturaTemporaraCopia[i][j] = new YIQ.EstruturaYIQ();
            }
        }        
        HSL.RGBparaHSL();
        YIQ.RGBparaYIQ();
    ...
    private void HistogramaOrigem(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_HistogramaOrigem
        if(imagemCarregadaDoDisco == null || FrameControleCheck.isShowing() || FrameControleRGB.isShowing() || FrameControleFiltros.isShowing() ||
           FrameControleLuzESombra.isShowing() || FrameControleHSL.isShowing() || FrameControleYIQ.isShowing() ||
           FrameControleMorfologico.isShowing() || progressFrame.isShowing()){
        }
        else{
        ...
        if(imagemCarregadaDoDisco != null){
            ...
            if (opcao == JFileChooser.APPROVE_OPTION) {
                ...
                try {
                    ...
                    ImageIO.write(imagemASerExibida, "PNG", new File(caminho));
                } catch (IOException ex) {
                    Logger.getLogger(FramePrincipal.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
    ...
    public static BufferedImage imagemCarregadaDoDisco, imagemASerExibida,
                  imagemASerSalvaEmDisco,imagemOriginalRedimensionadaSemAlteracao,
                  imagemCopia, imagemOriginalRedimensionadaParaReversao;
    public static javax.swing.JMenuItem jMenuItem1, jMenuItem2, jMenuItem3,
                  jMenuItem4, jMenuItem5, jMenuItem6,jMenuItem7, jMenuItem8,
                  jMenuItem9, jMenuItem10, jMenuItem11;
    public static boolean vermelhoCinza = false, verdeCinza = false,
                  azulCinza = false, verificarNegativo = false;
